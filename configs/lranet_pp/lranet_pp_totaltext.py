...
train_pipeline = [
    ...
    dict(type='RandomScaling', size=1280, scale=(1. / 10, 6. / 2)),
    ...
    dict(type='SquareResizePad', target_size=1280, pad_ratio=0.6),
...
data = dict(
    ...
    train=dict(
        ...
        dataset=dict(
            type=dataset_type,
            ann_file=data_root + 'totaltext/totaltext_train.json',
            img_prefix=data_root + 'totaltext/training',
    ...
    test=dict(
        ...
        img_prefix=data_root + 'totaltext',
        pipeline=test_pipeline,))
evaluation = dict(interval=500, metric='hmean-e2e',by_epoch=False)

# optimizer
optimizer = dict(type='AdamW', lr=2e-5, weight_decay=0.0001)

optimizer_config = dict(grad_clip=None)
lr_config = dict(policy='step', step=[5000,],by_epoch=False)
runner = {
            'type': 'IterBasedRunner',
            'max_iters': 10000
        }

custom_hooks = [dict(type='ExpMomentumEMAHook', momentum=0.0002, priority=49)]
