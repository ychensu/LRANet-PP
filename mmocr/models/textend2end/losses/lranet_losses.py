...
class SetCriterion(nn.Module):
    ...
    def _get_label_loss(self, outputs, targets, indices, num_inst, input_key, output_key):
        """
        Generic function to compute Classification loss (Focal Loss)
        Args:
            input_key: e.g., 'pred_logits' or 'pred_logits_tea'
            output_key: e.g., 'loss_ce_sparse' or 'loss_ce_sparse_tea'
        """
        assert input_key in outputs
        src_logits = outputs[input_key]
        ...
        return {output_key: class_loss}


    def _get_point_loss(self, outputs, targets, indices, num_inst, input_key, output_key):
        """
        Generic function to compute Regression loss (Smooth L1)
        Args:
            input_key: e.g., 'pred_points' or 'pred_points_tea'
            output_key: e.g., 'loss_point_sparse' or 'loss_point_sparse_tea'
        """
        assert input_key in outputs
        idx = self._get_src_permutation_idx(indices)

        src_points = outputs[input_key]
        ...
        loss_value = loss_points.mean(-1).sum() / num_inst

        return {output_key: loss_value}
    ...
    def loss_labels(self, outputs, targets, indices, num_inst, log=False):
        return self._get_label_loss(outputs, targets, indices, num_inst, 
                                    input_key='pred_logits', 
                                    output_key='loss_ce_sparse')

    def loss_points(self, outputs, targets, indices, num_inst):
        return self._get_point_loss(outputs, targets, indices, num_inst, 
                                    input_key='pred_points', 
                                    output_key='loss_point_sparse')

    def loss_labels_tea(self, outputs, targets, indices, num_inst, log=False):
        return self._get_label_loss(outputs, targets, indices, num_inst, 
                                    input_key='pred_logits_tea', 
                                    output_key='loss_ce_sparse_tea')

    def loss_points_tea(self, outputs, targets, indices, num_inst):
        return self._get_point_loss(outputs, targets, indices, num_inst, 
                                    input_key='pred_points_tea', 
                                    output_key='loss_point_sparse_tea')

    def get_loss(self, loss, outputs, targets, indices, num_inst, **kwargs):
        loss_map = {
            'labels': self.loss_labels,
            'points': self.loss_points,
            'labels_tea': self.loss_labels_tea,
            'points_tea': self.loss_points_tea,
        }
    ...
    def forward(self, outputs, targets):
        ...
        num_inst = torch.as_tensor([num_inst], dtype=torch.float, device=device)
...
class MinCostMatcher(nn.Module):
    ...
    def forward(self, outputs, targets):
        ...
        bs, k, hw = outputs["pred_logits_tea"].shape
        # We flatten to compute the cost matrices in a batch

        batch_out_prob = outputs["pred_logits_tea"].permute(0, 2, 1).sigmoid()  # [batch_size, num_queries, num_classes]
        batch_out_po =  outputs["pred_points_tea"].permute(0, 2, 1)
...
class LRALoss(nn.Module):
    ...
    def __init__(self, num_coefficients, path_lra, ohem_ratio=3.,
        ...
        losses = ['labels','points', 'labels_tea','points_tea']
        weight_dict = {'loss_ce_sparse': 2, 'loss_point_sparse': 0.1, 'loss_ce_sparse_tea': 2, 
                       'loss_point_sparse_tea': 0.1, "loss_ce_dense": 1, 'loss_point_dense': 4, }
    ...
    def _process_sparse_branch(self, raw_cls, raw_reg, stride, device):

        bs, _, h, w = raw_cls.shape
        processed_cls = raw_cls.permute(0, 2, 3, 1).contiguous().view(bs, h * w, -1)
        reg_lra = raw_reg.permute(0, 2, 3, 1).contiguous().view(-1, self.num_coefficients)
        p_pre = torch.matmul(reg_lra, self.U_t)
        locations = self.generate_locations(bs, w, h, device)
        p_pre[:, 0::2] += locations[:, 0].unsqueeze(1)
        p_pre[:, 1::2] += locations[:, 1].unsqueeze(1)
        p_pre = p_pre * stride
        processed_pts = p_pre.view(bs, h * w, -1)
        
        return processed_cls, processed_pts
    ...
    def forward(self, preds, _, p3_maps, p4_maps, p5_maps,polygons_area=None, lra_polys = None,**kwargs):
        ...
        losses = multi_apply(self.forward_single, preds, gts, self.steps, gt_polygons_areas)
        ...
        for idx, data in enumerate(losses):
            if idx == 0:
                ...
                tr_train_masks_list.append(data)

        tr_train_masks = torch.cat(tr_train_masks_list[0], dim=1)
        ...
        for i in range(len(preds)):
            stride = self.steps[i]
            cls_stu, pts_stu = self._process_sparse_branch(
                preds[i][2], preds[i][3], stride, device
            )
            all_cls_sparse.append(cls_stu)
            all_pts_sparse.append(pts_stu)

            cls_tea, pts_tea = self._process_sparse_branch(
                preds[i][4], preds[i][5], stride, device
            )
            all_cls_tea.append(cls_tea)
            all_pts_tea.append(pts_tea)

        flat_cls_sparse = torch.cat(all_cls_sparse, dim=1).permute(0, 2, 1)
        flat_pts_sparse = torch.cat(all_pts_sparse, dim=1).permute(0, 2, 1)
        flat_cls_tea = torch.cat(all_cls_tea, dim=1).permute(0, 2, 1)
        flat_pts_tea = torch.cat(all_pts_tea, dim=1).permute(0, 2, 1)
        ...
        for i in range(len(lra_polys)):
            ...
            target["labels"] = torch.zeros(num_polys).type(torch.int64).to(device)
            target["image_size"] = image_size
            target['points'] = torch.from_numpy(lra_polys[i]).to(device)
            new_targets.append(target)

        if cnt > 0:
            valid_cls_sparse = flat_cls_sparse[valid_batch_indices]
            valid_pts_sparse = flat_pts_sparse[valid_batch_indices]
            valid_cls_tea = flat_cls_tea[valid_batch_indices]
            valid_pts_tea = flat_pts_tea[valid_batch_indices]
            valid_masks = tr_train_masks[valid_batch_indices]

            output_sparse = {
                'pred_logits': valid_cls_sparse, 
                'pred_points': valid_pts_sparse,
                'pred_logits_tea': valid_cls_tea, 
                'pred_points_tea': valid_pts_tea,
                'tr_train_masks': valid_masks
            }
            
            loss_dict = self.criterion(output_sparse, new_targets)
        else:
            zero_loss = flat_pts_sparse.sum() * 0
            loss_dict = {
                'loss_ce_sparse': zero_loss,
                'loss_point_sparse': zero_loss,
                'loss_ce_sparse_tea': zero_loss,
                'loss_point_sparse_tea': zero_loss
            }
    ...
    def forward_single(self, pred, gt, downsample_rate=None,areas=None):
        ...
        p_gt = gt[:, :, :, 3:].view(-1, self.num_coefficients*2)
        ...
        if num_pos> 0:
            ...
            if self.with_center_weight:
                ...
                weight = torch.ones(num_pos,dtype=torch.float32, device=p_gt.device)
            ...
            p_pre = torch.matmul(lra_pred,self.U_t)
