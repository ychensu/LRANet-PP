...
def evaluate_method(gtFilePath, submFilePath, evaluationParams):
    ...
    def transcription_match(transGt,transDet,specialCharacters=str(r'!?.:,*"()·[]/\''),onlyRemoveFirstLastCharacterGT=True):
        ...
        # transGt = transGt.replace(" ", "")
        # transDet = transDet.replace(" ", "")
        ...
        if resFile in subm:
            ...
            for n in range(len(pointsList)):
                ...
                if len(transcription) > 24:
                    current_det_index = len(detPols) - 1
                    detDontCarePolsNum.append(current_det_index)
                        ...
                        if gtRectMat[gtNum] == 0 and detRectMat[detNum] == 0 and gtNum not in gtDontCarePolsNum and detNum not in detDontCarePolsNum :
                            if iouMat[gtNum,detNum]>evaluationParams['IOU_CONSTRAINT']:
                                ...
                                if evaluationParams['WORD_SPOTTING']:
                                    ...
                                    try:
                                        correct = transcription_match(gtTrans[gtNum].upper().replace(" ", ""),detTrans[detNum].upper().replace(" ", ""),evaluationParams['SPECIAL_CHARACTERS'],evaluationParams['ONLY_REMOVE_FIRST_LAST_CHARACTER'])==True
                                    except: # empty
                                        correct = False
