...
class ColorJitter:
    """
    Unified ColorJitter class merging YOLOX HSV, PIL Color Jitter, and Albumentations.
    Order: HSV (OpenCV) -> PIL Enhancements -> Albumentations -> Output
    """
    def __init__(self,
                 # 1. HSV Parameters
                 hue_delta: int = 5,
                 ...
                 value_delta: int = 30,
                 # 2. PIL Parameters (Brightness, Contrast, Sharpness, Color)
                 brightness_delta: float = 0.5,
                 contrast_delta: float = 0.5,
                 sharpness_delta: float = 0.8386,
                 color_delta: float = 0.5,
                 # 3. Albumentations Parameters
                 noise_p: float = 0.2,
                 blur_p: float = 0.2,
                 clahe_p: float = 0.2,
                 compression_p: float = 0.2,
                 compression_quality_lower: int = 85):
        ...
        # --- Config for PIL Jitter ---
        self.pil_transforms = [
            (ImageEnhance.Brightness, brightness_delta),
            (ImageEnhance.Contrast, contrast_delta),
            (ImageEnhance.Sharpness, sharpness_delta),
            (ImageEnhance.Color, color_delta)
        ]

        # --- Config for Albumentations ---
        self.albu_trans = A.Compose([
            A.OneOf([
                A.GaussNoise()
            ], p=noise_p),
            A.OneOf([
            ...
            A.CLAHE(clip_limit=2, p=clahe_p),
            A.ImageCompression(quality_lower=compression_quality_lower, p=compression_p),
        ])
    ...
    def __call__(self, results):
        img = results['img'] # Input is BGR

        # ============================================================
        # Step 1: HSV Augmentation (Requires BGR, Output BGR)
        # ============================================================
        ...
        # ============================================================
        # Step 2: PIL Augmentation (Requires RGB, Output RGB)
        # ============================================================
        # Convert BGR to RGB once here
        img = img[..., ::-1] 
        img_pil = Image.fromarray(img)

        # Randomize order or selection can be done here if needed, 
        # currently iterating all defined transforms with random intensity.
        rand_num = np.random.uniform(0, 1, len(self.pil_transforms))
        
        for i, (transformer_cls, alpha) in enumerate(self.pil_transforms):
            # r in [1-alpha, 1+alpha)
            r = alpha * (rand_num[i] * 2.0 - 1.0) + 1 
            img_pil = transformer_cls(img_pil).enhance(r)
        
        img = np.asarray(img_pil)

        # ============================================================
        # Step 3: Albumentations (Input RGB, Output RGB)
        # ============================================================
        # Albumentations works best with RGB, so we pass the RGB image directly
        img = self.albu_trans(image=img)['image']

        # ============================================================
        # Final Output (Convert RGB back to BGR)
        # ============================================================
        img = img[..., ::-1]
        results['img'] = img
        return results
    ...
    def __repr__(self):
        repr_str = self.__class__.__name__
        repr_str += f'(hue={self.hue_delta}, sat={self.saturation_delta}, val={self.value_delta}, '
        repr_str += f'pil_configs={len(self.pil_transforms)}, albu={self.albu_trans})'
        return repr_str
